...
# Retention window for uploaded sources, intermediate audio and finished jobs.
# CLEANUP_RETENTION_HOURS (from CLAUDE.md's env contract) is the knob users set;
# JOB_RETENTION_SECONDS stays as an internal override/back-compat escape hatch.
CLEANUP_RETENTION_HOURS = float(os.environ.get("CLEANUP_RETENTION_HOURS", "48"))
JOB_RETENTION_SECONDS = int(os.environ.get("JOB_RETENTION_SECONDS", str(int(CLEANUP_RETENTION_HOURS * 3600))))
...
# ---- Cloud billing: permanently disabled in this fork -------------------------
# Upstream OpenShorts optionally imports a `cloud/` package here for multi-user
# auth, billing and managed API keys. This fork is a private, single-user
# deployment: `cloud/` was removed entirely and BILLING_ENABLED is hardcoded
# off. Do not set a BILLING_ENABLED env var — there is nothing for it to load.
...
# MCP agent server (mcp_server.py) is out of scope for this private,
# single-user fork and was removed — it depended on the deleted cloud/
# package for its optional multi-user auth path.
...
async def run_job(job_id, job_data):
    ...
    try:
        ...
        if returncode == 0:
            ...
            # No cloud backup in this fork — everything stays on local disk
            # and is swept by the retention cleanup job (see cleanup.py).
...
async def edit_clip(
    ...
    # No BYOK in this fork: any client-supplied api_key is ignored, the
    # Gemini key always comes from the server's own environment.
    ...
    # Billing metering is permanently disabled in this fork; reserve_managed_action
    # is a no-op self-host no-op (BILLING_ENABLED is always False here).
